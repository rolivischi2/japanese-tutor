...
TODO: Describe each pattern clearly with the dictionary-notation number, pitch shape, and example words:

### Heiban (平板) — Pattern 0: No drop
...
### Odaka (尾高) — Drop after the last mora of the word
TODO: Dictionary notation: n = word length. Pitch shape: L on mora 1, H from mora 2 to last mora, L on following particle. Example: はな (nose, 2 for a 2-mora word) = LH+drop. Distinguishable from heiban only by particle pitch.

## Heiban/Odaka Isolation Ambiguity

TODO: Explain the practical problem: in isolation, heiban (0) and odaka (n=length) are acoustically identical. They diverge only when a particle follows (heiban keeps particle high; odaka drops to low on the particle). Implication: always learn pitch with a trailing particle, e.g. はなが (flower-ga) vs はなが (nose-ga).

## Dictionary Notation

TODO: Explain that dictionaries (NHK, Kanjium) use a drop-position number: 0 = heiban, 1 = atamadaka, n = drop after mora n. This notation is used in `vocab/pitch-accent-overlay.csv` and `pronunciation/10-pitch-drills.json`.

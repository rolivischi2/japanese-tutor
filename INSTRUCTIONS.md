# Japanese Conversational-Mastery Knowledge Base — Repository Blueprint for Roland
...
- **Kana-first for ~2 weeks, then no-kanji modules 00–03, then context-tied kanji from Module 04 onward** mapped 1:1 to vocab already in your SRS — this matches Roland's "recognition over production" goal and avoids the WaniKani-style isolated-kanji trap that wastes time for a conversational learner.
- **Tool stack: Anki + Kaishi 1.5k deck + Yomitan + Migaku-style mining + Kotu/Dogen pitch-accent training + Comprehensible Japanese (Yuki) and Nihongo con Teppei for input + Preply for output**, all glued together by versioned Markdown/JSON files in a single repo Claude Code can extend deterministically.
...
- **Polite-first but plain-form-fast.** Modules 01–05 use ます/です so Roland can use what he learns in Preply lessons next week. Module 06 introduces plain forms aggressively, and from Module 07 onward everything is taught in *both* registers simultaneously, with the casual register marked as primary for listening comprehension and the polite register marked as primary for output.
...
- **Pitch accent is taught from Module 00**, not deferred. Hungarian L1 + German L2 speakers will impose first-syllable intensity stress unless explicitly trained — this is by far the single biggest accent-killer.
...
### 1.3 Hungarian + German background — pronunciation prognosis

Roland's L1 (Hungarian) and L2 (German/Swiss German) give him a phonological starting kit that is unusually favourable for Japanese, with a few specific traps. Verdicts derived from contrastive analysis of Wikipedia's Hungarian phonology and Japanese phonology articles, Siptár & Törkenczy *The Phonology of Hungarian* (Oxford, 2000), Szende *JIPA* 1994, Szeredi WCCFL 2010, and the L2-Japanese phonological literature:

| Feature | Roland's prognosis |
|---|---|
| Phonemic geminate consonants (っ, e.g. きって vs きて) | **Strong advantage.** Per the Hungarian phonology article, Hungarian is notable for "the frequent occurrence of geminate consonants… almost every consonant may be geminated" (e.g. *vissza*, *meggy* vs *megy*). Wikipedia's gemination article groups Japanese and Hungarian as typologically equivalent on this contrast. Swiss German also preserves consonant length, unlike Standard German. |
| Phonemic vowel length (おばさん vs おばあさん) | **Strong advantage.** Hungarian distinguishes seven short/long vowel pairs phonemically (*kor* vs *kór*); however, Hungarian length pairs also differ in vowel quality, while Japanese long vowels are pure duration. Risk: Roland may unconsciously alter quality when lengthening. |
| ら-row (alveolar tap [ɾ]) | **Strong advantage.** Tar (2017, *Clinical Linguistics & Phonetics*) documents that Hungarian /r/ "often does not surface as a trill and present more as a tap." The Hungarian tap allophone is essentially identical to the Japanese tap. He must only suppress the trilled allophone. |
| つ [tsɯ] | **Advantage.** Hungarian "c" = [ts] occurs before any vowel (*cukor*). |
| Japanese /a, i, o/ | Clean Hungarian mappings. |
| Japanese /e/ [e̞] | **Trap.** Hungarian short "e" is too open [ɛ]. He needs to use the quality of Hungarian long "é" but with short duration. |
| Japanese /u/ [ɯ̜/ʉ] (compressed, near-unrounded) | **Trap.** Hungarian /u/ is fully rounded. Single most salient vowel difficulty. |
| ふ [ɸɯ] (bilabial fricative) | **Difficulty.** Not in Hungarian. Will substitute labiodental [f]. |
| ひ [çi] (palatal fricative) | Mild difficulty in Hungarian; **but** Swiss German /ç/ (*ich-Laut*) is an exact match. Use the German habit. |
| Pitch accent | **Major difficulty.** Szeredi (2010 WCCFL) shows Hungarian stress is intensity-driven and locked to the first syllable: "vowel intensity is the main cue… the first syllable had the highest intensity and the intensity peaks on the following vowels decreased gradually." Roland will tend to bash the first mora of every word. Explicit drilling from week 1 is essential. |
| Mora-timing | Mild advantage over English speakers — Siptár & Törkenczy (2000) classify Hungarian as having "syllable-timed prosody (cf. Roach 1982, Crystal 1995)", which transfers better than stress-timing, but Hungarian syllables are unequal in weight whereas Japanese morae are equal. |

The repo's pronunciation track encodes all of this explicitly.
...
**`README.md`** — One-page orientation. *What this repo is*: a personal Japanese curriculum aimed at conversational mastery, not JLPT. *Who it's for*: Roland — adult, technical, Hungarian L1, German L2, Mac user, Preply student. *How to use it*: pick the active module from `INDEX.md`, follow `00-overview.md`, do exercises, log in `progress/progress.md`. *Daily routine*: 15 min Anki + 20 min current-module + 20 min listening + 10 min shadowing/output ≈ 65 min/day. *Dependencies*: Anki, Yomitan, Kaishi 1.5k, optional Migaku.
...
**`preply-lesson-log.md`** — One row per lesson: date, tutor, topics, mistakes flagged, homework. Claude Code can inject flagged mistakes into a review queue deck.
...
**`01-polite-vs-plain-strategy.md`** — Explicit treatment of the debate. Position: polite-first in M01–M05 because it lets him use Preply lessons immediately; plain-form crash course in M06 because by then he has enough exposure to recognise it everywhere. Plain form *for input* from day 1 via Comprehensible Japanese videos. Concrete rule: from M06, every new structure is shown in side-by-side polite/plain tables.
...
**`03-srs-strategy.md`** — Anki configuration recipe: new cards/day = 10 (M00–04), then 15; max reviews/day = 200; FSRS scheduler default. Two decks: `Roland::Core` (Kaishi 1.5k + module-vocab) and `Roland::Mining` (Yomitan-added). One kanji deck `Roland::Kanji` (recognition only; "what does this kanji mean?" prompt; reading on back). Suspend mined cards seen only once in 2 weeks of immersion. Burying related cards: ON.
...
**`05-self-talk-protocol.md`** — 30 daily prompts (commute, today's lunch, work issue, weekend plan, weather opinion) graded by module. Roland records himself in Voice Memo for 60s, transcribes into `output-practice/self-talk-prompts.md`.
...
- **Preply prep**: "Given the topic [X], produce 10 example questions a tutor might ask me in plain Japanese, and 5 prompt answers using only grammar from Modules 00–06."
...
**`02-the-five-vowels.md`** — IPA values + Hungarian/German mapping (see §1.3). Drills for the /u/ trap (compressed, not protruded): record yourself saying すみません without rounding lips.

**`03-tricky-consonants.md`** — For each of り, つ, ふ, ひ, し, ち, じ: IPA, mouth diagram (text description), 6 minimal-pair drills, Hungarian/German transfer note. Special handling for ふ (substitute labiodental [f] flagged as a Hungarian-default error).
...
**`07-hungarian-transfer-notes.md`** — Full content from §1.3 prognosis table with detailed drills. Emphasis on suppressing first-mora intensity (Hungarian fixed-stress interference per Szeredi 2010). Drill: pronounce はし three ways (HL, LH, LH+drop) with deliberately equal loudness on every mora.

**`08-german-swiss-transfer-notes.md`** — Swiss German alveolar trill /r/ is closer to ら than Standard German uvular /ʁ/; use it. Swiss German preserves consonant length (an extra advantage over Standard German). German [ç] in *ich* is an exact match for Japanese ひ.
...
#### Module 01 — Copula Basics (Roland's current location)
...
- **First kanji (~30)**: 一二三四五六七八九十百千万 (numbers); 日月火水木金土 (days); 人, 私, 何, 行, 食, 飲, 見, 聞, 来, 大, 小. Each tied to vocab Roland already knows.
- ~120 vocab.
...
**`tier-2-expand-700.csv`** — JLPT N4-ish + remaining Kaishi 1.5k + topical buckets: work (15 IT terms — Roland is a developer), hobbies, feelings, opinions, travel.
...
- **No isolated kanji study.** Kanji are introduced only when Roland already knows a vocab word using them. This avoids the WaniKani problem of "learning readings out of context that don't stick" (documented across the donkuri immersion guide and the LearnKanji guide).
...
- **`common-mistakes.md`** — Running catalogue Roland builds from Preply + AI corrections. Append-only.
...
- **From M08–09**: native-pace interest podcasts. For Roland: *Rebuild.fm* (Japanese IT podcast).
...
**`preply-prep-templates.md`** — Before each Preply lesson: 10-min prep where Roland reads next module's `dialogues.md`, generates 5 tutor questions, lists 3 concepts to correct.
...
**`yomitan-setup.md`** — Install Yomitan in Safari/Firefox/Chrome/Edge (Yomitan is officially supported on the latter three). Recommended dictionaries: JMdict (Jitendex), KANJIDIC, Kanjium pitch accent, JPDB frequency. Enable Anki integration pointed at `Roland::Mining` deck with the Lapis card model (or donkuri's fork). Optional: Local Audio Server for offline pitch audio.
...
| Output | **Preply** (current) | iTalki | All modules |
...
- Preply tutor reports Roland is over-translating mentally from English → push input ratio +10% and skip output sessions for a week.
- Pitch-accent self-recordings still sound flat at end of Module 03 → escalate Dogen from M04 to M02.
- Roland reaches end of Module 07 ahead of schedule → run M08–M10 in parallel (they don't strongly depend on each other).
...
3. **Populate Module 00 fully** (kana drills, mnemonics referenced not duplicated, minimal-pair drills) — the only module 100% built at scaffold time. All later modules are filled in chapter-by-chapter as Roland progresses.
...
This produces a self-extending repo: when Roland finishes Module N, he asks Claude Code "build Module N+1's drills and dialogues based on the overview" and Claude Code reads the conventions in `CLAUDE.md` and produces consistent content.
...
2. **Parallel with Preply lessons, add Comprehensible Japanese (Yuki) as daily passive listening.** 10 min/day during commute or cooking. Frame: "let your ears catch up to your mouth."
3. **Tell your Preply tutor your plan.** Show them Module 01's overview. Ask them to drill copula and は in plain conversation for 2–3 weeks rather than rushing ahead.
...
6. **At end of Module 06, switch the Preply lesson register to predominantly plain form.** This is the gear shift toward real conversational ability. Tell the tutor explicitly.
...
- **The Hungarian-to-Japanese phonology mapping is contrastive analysis, not empirical study.** I found no published L1-Hungarian → L2-Japanese phonetics paper specifically; the prognoses in §1.3 are well-founded inferences from independently described L1 and L2 systems (Wikipedia phonology articles, Siptár & Törkenczy 2000, Szeredi 2010, Tar 2017) plus general L2-prosody-transfer findings. Treat them as hypotheses to test on himself in the first month.
